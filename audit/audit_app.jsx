...
/* Section de cours contrôlée (même look qu'ACollapse, mais pilotée par le
   parent pour permettre le sommaire cliquable et « tout déplier »). */
function AuditCourseSection({ id, num, title, open, onToggle, children }) {
  return (
    <div id={id} className="mb-3 rounded-xl border border-slate-200 bg-white overflow-hidden scroll-mt-3">
      <button onClick={onToggle} className="w-full flex items-center gap-2.5 px-4 py-3 text-left hover:bg-slate-50">
        <span className="w-6 h-6 rounded-lg bg-violet-50 border border-violet-100 text-violet-700 text-[11px] font-bold flex items-center justify-center shrink-0">{num}</span>
...
function AuditCourse({ std, course, onBack }) {
  ...
  const [openSecs, setOpenSecs] = useState(() => sections.map((_, i) => i === 0));
  const allOpen = openSecs.every(Boolean);
  const toggleSec = (i) => setOpenSecs((o) => o.map((v, j) => (j === i ? !v : v)));
  const gotoSec = (i) => {
    setOpenSecs((o) => o.map((v, j) => (j === i ? true : v)));
    setTimeout(() => { const el = document.getElementById("course-sec-" + i); if (el) el.scrollIntoView({ behavior: "smooth", block: "start" }); }, 60);
  };
  ...
  return (
    <div className="max-w-[920px] mx-auto">
      ...
      {/* ── En-tête + résumé : la norme en bref, lisible d'abord ── */}
      <div className="rounded-2xl border overflow-hidden mb-4" style={{ borderColor: color + "55" }}>
        <div className="p-5 pb-4" style={{ background: color + "12" }}>
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className="text-white text-sm font-bold px-3 py-1 rounded-lg" style={{ background: color }}>{std.code}</span>
            {course.niveau && <span className="text-[11px] font-medium text-slate-600 bg-white/70 border border-slate-200 px-2 py-1 rounded-lg">{course.niveau}</span>}
            {course.duree && <span className="text-[11px] text-slate-500">⏱️ {course.duree}</span>}
          </div>
          <h2 className="text-2xl font-bold text-slate-800 leading-tight">{std.title_fr}</h2>
      ...
      {/* ── À connaître absolument : synthèse + repères + mnémo, avant tout ── */}
      {((course.synthese || []).length > 0 || (course.stats || []).length > 0 || (course.mnemo && course.mnemo.items)) && (
        <div className="rounded-2xl border border-violet-200 bg-white overflow-hidden mb-4 shadow-sm">
          <div className="px-5 py-3 bg-gradient-to-r from-violet-50 to-fuchsia-50 border-b border-violet-100">
            <span className="font-bold text-violet-900">🎯 À connaître absolument</span>
            <span className="text-[11px] text-violet-500 ml-2">l'examen se joue ici</span>
          </div>
          <div className="p-5 pt-4 space-y-4">
            {(course.synthese || []).length > 0 && (
              <div className="grid sm:grid-cols-2 gap-x-5 gap-y-2.5">
                {course.synthese.map((it, i) => (
                  <div key={i} className="flex items-start gap-2.5">
                    <span className="w-6 h-6 rounded-lg text-[11px] font-bold flex items-center justify-center shrink-0 mt-0.5"
                      style={{ background: color + "18", color, border: `1px solid ${color}33` }}>{i + 1}</span>
                    <span className="text-sm text-slate-700 leading-snug font-medium"><MdInline text={it} /></span>
                  </div>
                ))}
              </div>
            )}
            {(course.stats || []).length > 0 && (
              <div>
                <div className="text-[11px] font-bold uppercase tracking-wide text-slate-400 mb-1.5">📊 Repères chiffrés</div>
                <div className="flex flex-wrap gap-1.5">
                  {course.stats.map((s, i) => (
                    <span key={i} title={s.sub || ""} className="inline-flex items-baseline gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5">
                      <span className="text-sm font-bold text-indigo-700"><MdInline text={s.value} /></span>
                      <span className="text-[11px] text-slate-600"><MdInline text={s.label} /></span>
                    </span>
                  ))}
                </div>
              </div>
      ...
      {/* ── Sommaire cliquable du cours ── */}
      {sections.length > 1 && (
        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 mb-4">
          <div className="flex items-center justify-between gap-2 mb-2">
            <span className="text-[11px] font-bold uppercase tracking-wide text-slate-400">🧭 Sommaire du cours — {sections.length} sections</span>
            <button onClick={() => setOpenSecs(sections.map(() => !allOpen))} className="text-[11px] font-semibold text-violet-600 hover:text-violet-800">
              {allOpen ? "Tout replier" : "Tout déplier"}
            </button>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {sections.map((s, i) => (
              <button key={i} onClick={() => gotoSec(i)}
                className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 hover:border-violet-300 hover:bg-violet-50 px-2 py-1 text-[12px] text-slate-700 transition-colors">
                <span className="font-mono text-[10px] text-violet-600 font-bold">{i + 1}</span>{s.titre}
              </button>
            ))}
          </div>
        </div>
